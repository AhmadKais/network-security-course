...
**⁦running-config⁩** – ההגדרות **הפעילות עכשיו**, ב-**⁦RAM**.⁩ כל פקודה שמקלידים משנה אותה **מיד**, והמכשיר מתנהג לפיה. **נעלמת בכיבוי.** **⁦startup-config⁩** – ההגדרות **השמורות**, ב-**⁦NVRAM**.⁩ נטענת בהפעלה. משתנה **רק** ב-`⁦write memory⁩` (או `⁦copy running-config startup-config⁩`).
**תרחיש אובדן:** תלמיד מגדיר מתג ⁦40⁩ דקות – ⁦VLAN⁩-ים, ⁦Trunk⁩-ים, ⁦STP.⁩ הכול עובד. הוא מכבה ומדליק "לבדוק שזה נשמר". המתג עולה **ריק**. הוא לא הקליד `⁦write memory⁩`, וה-⁦running-config⁩ נמחק עם הכיבוי.
**תרחיש ניצול מכוון:** מנהל מנסה שינוי מסוכן ברשת חיה (למשל שינוי ב-⁦STP).⁩ השינוי משתבש והרשת מתנהגת מוזר. במקום לפרק את השינוי ביד ולחפש מה בדיוק שבור – הוא **לא שומר** ומקליד `⁦reload⁩`. תוך דקה המכשיר חוזר להגדרה הטובה האחרונה. הרגל: `⁦write memory⁩` אחרי כל שינוי **שעובד**; לא לשמור שינוי **שלא עובד**.
...
**שיטה: שכבה-שכבה מלמטה למעלה, פקודה אחת לכל שכבה, הראשונה שנכשלת מאתרת.**
**שלב ⁦1⁩ – במחשב:** `⁦ipconfig /all⁩` – יש כתובת תקינה? (⁦169.254 = DHCP⁩ נכשל → בעיה בשכבה ⁦2⁩ או ⁦DHCP.)⁩ **שלב ⁦2⁩:** `⁦ping⁩ <שער>` – מגיע לשער? (לא → ⁦ARP/SVI/HSRP/VLAN⁩ – השכבה המקומית.) **שלב ⁦3⁩:** `⁦ping⁩ <שרת IP>` – מגיע ליעד לפי כתובת? (לא → ניתוב או ⁦NAT⁩ או ⁦ACL.)⁩ **שלב ⁦4⁩:** `⁦tracert⁩ <שרת>` – **איפה** נתקע? הקפיצה האחרונה שענתה = הנתב שלפני הבעיה. **שלב ⁦5⁩:** `⁦ping⁩ <שם>` – אם ⁦IP⁩ עובד ושם לא → ⁦DNS.⁩
**בנתב שנמצא בשלב ⁦4⁩:** `⁦show ip interface brief⁩` (הממשקים למעלה?), `⁦show ip route⁩` (יש נתיב ליעד? לחזרה?), `⁦show ip nat translations⁩` (אם זה הקצה). **במתג אם נכשל בשלב ⁦2⁩:** `⁦show vlan brief⁩` (הפורט ב-⁦VLAN⁩ הנכון?), `⁦show interfaces trunk⁩` (ה-⁦VLAN⁩ מותר?), `⁦show spanning-tree⁩` (חסום?), `⁦show standby brief⁩` (מי ⁦Active⁩?).
כל בדיקה תלויה רק בקודמות, אז הכישלון הראשון מצביע ישירות על השכבה. רק אז פותחים קונפיגורציה – ורק של המכשיר הרלוונטי.
...
**⁦err-disabled⁩** – מצב שבו המתג **כיבה פורט אוטומטית** בגלל הפרת מדיניות. הפורט כבוי מנהלתית **ולא מתאושש לבד** – גם אם מוציאים את המכשיר הבעייתי.
**שלושה מנגנונים:** (⁦1) **BPDU Guard⁩** – פורט ⁦Access⁩ (עם ⁦PortFast)⁩ קיבל ⁦BPDU⁩ → מישהו חיבר מתג. (⁦2) **Port Security⁩** במצב `⁦shutdown⁩` – יותר כתובות ⁦MAC⁩ מהמותר, או כתובת לא מורשית. (⁦3) **Storm Control⁩** עם `⁦action shutdown⁩` – יותר מדי שידורים מהפורט. (גם: אי-התאמת ⁦Duplex, Link Flap.)⁩
**הנוהל:** (⁦1)⁩ `⁦show interfaces status err-disabled⁩` – לאשר ולראות **איזה** מנגנון. (⁦2)⁩ **לתקן את הסיבה** – לנתק את המתג הזר / להסיר את המכשיר העודף – אחרת הפורט ייסגר שוב מיד. (⁦3)⁩ `⁦interface FaX⁩` → **`⁦shutdown⁩`** ואז **`⁦no shutdown⁩`** – שני הפקודות, בסדר הזה. `⁦no shutdown⁩` לבד לא עושה כלום כי הפורט לא ב-⁦shutdown.⁩ אופציונלי: `⁦errdisable recovery cause⁩ <x>` + `⁦errdisable recovery interval 300⁩` – התאוששות אוטומטית אחרי ⁦5⁩ דקות.
...
**מועיל:** ⁦CDP⁩ מודיע לכל שכן ישיר: שם המכשיר, דגם, גרסת ⁦IOS⁩, ומאיזה פורט מחובר. `⁦show cdp neighbors detail⁩` ממפה רשת לא מוכרת תוך דקה – בלי תיעוד, בלי ללכת לחדר השרתים. ומזהה אי-התאמות: `%⁦CDP-4-NATIVE_VLAN_MISMATCH⁩` מגיע ממנו.
**מסוכן:** ⁦CDP⁩ **בלי אימות** ו**לכל מי שמחובר**, כל ⁦60⁩ שניות. תוקף שמחבר מחשב לפורט ⁦Access⁩ ומאזין מקבל: דגם המתג וגרסת התוכנה = בדיוק מה שצריך כדי לבחור ⁦exploit⁩ מתאים. וגם: מבנה הרשת, שמות, כתובות ניהול.
**המדיניות הנכונה: סלקטיבית.** `⁦no cdp enable⁩` על **פורטי המשתמשים** (⁦Fa0/1⁩–⁦21)⁩ – שם אין שכן ⁦Cisco⁩, רק סיכון. **להשאיר** על ⁦Trunk⁩-ים ו-⁦uplinks⁩ – שם הוא שימושי ואין תוקף (בהנחה שהתשתית מאובטחת). **למה לא `⁦no cdp run⁩` גלובלי:** מאבדים את הכלי בכל מקום, כולל איפה שהוא בטוח ושימושי. סלקטיבי = שקלנו תועלת מול סיכון – וזו התשובה שמקבלת ניקוד בבחינה.
...
**⁦169.254.x.x = APIPA⁩** – המחשב **לא הצליח לקבל כתובת מ-⁦DHCP⁩** ונתן לעצמו כתובת זמנית. הבעיה בדרך בין המחשב לשרת ה-⁦DHCP.⁩
**במחשב:** (⁦1)⁩ `⁦ipconfig /release⁩` + `⁦ipconfig /renew⁩` – לנסות שוב. עדיין ⁦169.254⁩ → ממשיכים.
**במתג הגישה:** (⁦2)⁩ `⁦show interfaces status⁩` – הפורט ⁦up⁩? ב-⁦VLAN⁩ הנכון? לא `⁦err-disabled⁩`? (⁦3)⁩ `⁦show vlan brief⁩` – ה-⁦VLAN⁩ קיים? (⁦4)⁩ `⁦show interfaces trunk⁩` – ה-⁦VLAN⁩ הזה ב-⁦allowed⁩ על ה-⁦uplink⁩? (⁦5)⁩ `⁦show spanning-tree interface FaX⁩` – הפורט ⁦FWD⁩? יש ⁦PortFast⁩? (בלי ⁦PortFast⁩ – לחכות ⁦30⁩ שניות ולנסות שוב.) (⁦6)⁩ `⁦show ip dhcp snooping⁩` – אם ⁦snooping⁩ מופעל, ה-⁦uplink⁩ לשרת **⁦trusted⁩**? (⁦7)⁩ `⁦show mac address-table⁩` – המתג למד את המחשב?
**במתג ההפצה / הנתב:** (⁦8)⁩ `⁦show ip interface brief⁩` – ה-⁦SVI⁩ של ה-⁦VLAN up/up⁩? (⁦9)⁩ `⁦show ip dhcp pool⁩` – יש מאגר ל-⁦VLAN⁩ הזה? לא גמור? (⁦10)⁩ אם השרת ב-⁦VLAN⁩ אחר: `⁦show run interface VlanX⁩` – יש `⁦ip helper-address⁩`? על ה-⁦SVI⁩ של **הלקוחות**? (⁦11)⁩ `⁦show ip dhcp binding⁩` / `⁦show ip dhcp conflict⁩` – מה השרת רואה?
**בשרת (אם ייעודי):** השירות פועל? המאגר מוגדר ונלחץ ⁦Add⁩? השרת עצמו מגיע לשער?
הכישלון הראשון בסדר הזה הוא הבעיה.
