...
2: eth0: <BROADCAST,MULTICAST,UP> mtu 1500 ...
    link/ether 08:00:27:ab:cd:ef          # ← זו כתובת ה-MAC שלכם (תעודת הזהות)
    inet 10.0.2.15/24 ...                  # ← זו כתובת ה-IP שלכם (הכתובת למשלוח)
```
...
$ traceroute google.com
 1  10.0.2.2        0.4 ms      # הנתב שבבית / בכיתה
...
```
$ ip route
default via 10.0.2.2 dev eth0        # "כל מה שלא מקומי → שלח לשער 10.0.2.2"

$ ip neigh
10.0.2.2 dev eth0 lladdr 52:54:00:12:35:02 REACHABLE   # טבלת ARP: IP ↔ MAC של השער
```
...
```
# תלמיד א' — השרת (צריך לדעת את ה-IP של עצמו מ- ip addr)
$ nc -lvnp 4444
...
```
# טרמינל 1 — שרת מדומה שמקשיב בדלת 8080
$ nc -lvnp 8080
```

```
# טרמינל 2 — curl "דופק" על אותה דלת
...
```
GET /hello HTTP/1.1          # "תן לי את הדף /hello"
Host: localhost:8080
User-Agent: curl/8.5.0        # מי מבקש (כאן curl; בדפדפן יהיה Chrome/Firefox)
Accept: */*
```
...
> host: example.com
< HTTP/2 200                   # מה שהשרת עונה: 200 = "בבקשה, הכול תקין"
< content-type: text/html
<!doctype html> ...            # דף ה-HTML עצמו – בדיוק מה שהדפדפן היה מצייר
```
