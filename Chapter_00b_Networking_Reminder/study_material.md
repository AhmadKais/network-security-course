...
הטבלה מראה *מה* יש בכל שכבה. אבל איך המידע באמת זז ביניהן? כשאתם שולחים הודעה, כל שכבה "עוטפת" את מה שקיבלה מהשכבה שמעליה בכותרת (⁦Header)⁩ משלה – בדיוק כמו מכתב שנכנס למעטפה, שנכנסת לשקית דואר, שנכנסת לארגז משלוח. התהליך נקרא **אנקפסולציה (⁦Encapsulation)⁩**:

```mermaid
graph TB
  A["Application – Data (ההודעה שלכם)"] --> T["Transport – TCP header + Data = Segment"]
  T --> N["Network – IP header + Segment = Packet"]
  N --> L["Link – Ethernet header + Packet + FCS = Frame"]
  L --> P["Physical – bits 0/1 על הכבל"]
```

בצד המקבל התהליך רץ הפוך (⁦De-encapsulation)⁩: כל שכבה מסירה את הכותרת שלה ומעבירה את התוכן כלפי מעלה. זו הסיבה שכתובת ה-⁦IP⁩ (שכבה ⁦3)⁩ וכתובת ה-⁦MAC⁩ (שכבה ⁦2)⁩ יושבות ב*כותרות שונות* של אותה חבילה בדיוק – ה-⁦IP⁩ אומרת "לאיזה בית", וה-⁦MAC⁩ אומרת "לאיזה כרטיס רשת בקטע הרשת הנוכחי".

> 💡 **למה השכבות מעניינות אותנו באבטחה?**
>
> לכל שכבה יש התקפות משלה – וגם הגנות משלה. זהו למעשה עמוד השדרה של כל הקורס: 
> - **שכבה ⁦2 (Link)⁩:** הרעלת ⁦ARP⁩, הצפת ⁦MAC⁩ ⇐ נלמד בפרק ⁦6.⁩
> - **שכבה ⁦3 (Network)⁩:** זיוף כתובת ⁦IP⁩, סינון בעזרת ⁦ACL⁩ ⇐ פרקים ⁦1⁩ ו-⁦4.⁩
> - **שכבה ⁦4 (Transport)⁩:** הצפת ⁦SYN⁩, סריקת פורטים ⇐ פרקים ⁦1⁩ ו-⁦5.⁩
> - **שכבה ⁦7 (Application)⁩:** ⁦Phishing⁩, נוזקות, הזרקות ⇐ פרקים ⁦1⁩ ו-⁦4.⁩ מכיוון שהתקפה חיה בשכבה מסוימת, לרוב אפשר לעצור אותה בשכבה אחרת – ולכן בונים הגנה ב**שכבות** (⁦Defense in Depth)⁩, ולא נשענים על חומה אחת בלבד.
