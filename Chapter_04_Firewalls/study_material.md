...
### 🖼️ איור: חומת אש כמו שומר בשער

```mermaid
flowchart LR
    NET["🌍 Internet"] -->|"traffic"| FW["👮 Firewall<br/>checks the rulebook"]
    FW -->|"✅ allowed"| IN["🏠 Internal network"]
    FW -->|"❌ denied"| DROP["🗑️ dropped"]
```

_השומר בודק כל אחד מול "ספר הכללים" (המדיניות): מי שמותר נכנס, מי שלא — נחסם._
