...
> 📖 **VPNFilter (2018) – חצי מיליון נתבים ביתיים נחטפו**
>
> נוזקה בשם VPNFilter הדביקה מעל 500,000 נתבים ביתיים וקטנים בעולם (Linksys, MikroTik, Netgear, TP-Link ועוד). היכולות שלה: ריגול אחר כל התעבורה שעוברת בנתב, גניבת סיסמאות, ואף "הריגת" הנתב (bricking) בפקודה אחת. ה-FBI ייחס אותה לתקיפה מדינתית והשתלט על שרת השליטה (C2) שלה כדי לעצור אותה. **איך היא נכנסה?** חולשות ידועות שלא עודכנו + סיסמאות ברירת מחדל שלא שונו. לקח: נתב ביתי הוא מחשב מלא – בלי הקשחה ועדכון הוא הדלת הקלה ביותר פנימה.

> 📖 **הדגל האמריקאי על 168,000 נתבים (Cisco Smart Install, 2018)**
>
> קבוצת האקרים ניצלה שירות ניהול של סיסקו בשם **Smart Install** (פורט TCP 4786) שנשאר **פתוח וחשוף לאינטרנט** בעשרות אלפי ארגונים. הם מצאו את המכשירים החשופים דרך מנוע החיפוש Shodan, איפסו את הקונפיגורציה של כ-168,000 נתבים, ובחלקם כתבו קונפיג עם דגל אמריקאי ב-ASCII וההודעה "Don't mess with our elections". נפגעו ספקיות אינטרנט באיראן וברוסיה. **לקח:** שירות ניהול מיותר שנשאר פתוח = הזמנה. זו בדיוק הסיבה שמכבים כל שירות שלא נחוץ (`no ...`) ומריצים AutoSecure.

> 📖 **Shodan – "גוגל של המכשירים המחוברים"**
>
> Shodan הוא מנוע חיפוש שסורק כל הזמן את האינטרנט ומקטלג מכשירים חשופים: נתבים, מתגים, מצלמות אבטחה, ואפילו בקרים תעשייתיים. אפשר לחפש בו מכשירים עם SNMP "public", עם Telnet פתוח, או עם סיסמת ברירת מחדל – ולקבל רשימה של אלפים. תוקפים משתמשים בו לשלב הסיור (פרק 1); אנשי אבטחה משתמשים בו כדי לגלות מה **הארגון שלהם** חושף בטעות. כל פעולה שעשינו בפרק הזה – SSH במקום Telnet, SNMPv3, כיבוי שירותים – מורידה אתכם מהרשימה של Shodan.

> ✏️ **תרגיל 5 – קישור סיפור להקשחה**
>
> לכל סיפור מהעולם האמיתי לעיל, איזו פקודת/פעולת הקשחה מהפרק הייתה מונעת אותו?
>
> **תשובה:** VPNFilter: שינוי סיסמת ברירת מחדל (`enable secret`, `username secret`) + עדכון IOS. Cisco Smart Install: כיבוי השירות המיותר (`no vstack` / כיבוי שירותים + AutoSecure). Shodan: SSH במקום Telnet, SNMPv3, וכיבוי כל שירות פתוח מיותר.

> ✏️ **תרגיל 6 – תכנון הקשחה**
>
> אתם מנהלי הרשת של בית ספר עם 3 נתבים. כתבו 6 צעדי הקשחה שתבצעו על כל נתב, לפי סדר עדיפות, ונמקו את הראשון.
>
> **תשובה:** דוגמה: 1. `enable secret` + `username ... secret` (בלי זה כל השאר חסר ערך – אין הגנה בסיסית). 2. SSH בלבד (`transport input ssh` + `login local`), כיבוי Telnet. 3. `login block-for` נגד ניחוש סיסמאות. 4. syslog + NTP לשרת מרכזי. 5. SNMPv3, כיבוי HTTP/CDP מיותרים. 6. banner אזהרה + גיבוי הקונפיג.
