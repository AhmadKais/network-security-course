...
> - **גיבוב (⁦Hashing)⁩** – "טביעת אצבע" של המידע: פונקציה **חד-כיוונית** שאי אפשר להפוך. מטרה: לבדוק ש**המידע לא שונה (שלמות)**.
>
> **אנלוגיה:** קידוד = לכתוב מילה הפוך (כל אחד קורא). הצפנה = כספת עם מפתח (רק בעל המפתח פותח). גיבוב = לטחון פרי לשייק (אי אפשר להחזיר את הפרי, אבל אותו פרי תמיד נותן אותו שייק).   
