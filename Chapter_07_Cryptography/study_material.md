...
### 🖼️ איור: הצפנה כמו תיבה נעולה

```mermaid
flowchart LR
    P["📄 Plaintext<br/>readable"] -->|"🔒 lock with a KEY"| C["🔀 Ciphertext<br/>scrambled"]
    C -->|"🔑 unlock with a KEY"| P2["📄 Plaintext<br/>readable again"]
    EVE["🦹 Attacker without the key"] -.->|"sees only 🔀"| C
```

_מי שאין לו את המפתח רואה רק ערבוב חסר משמעות. השאלה הגדולה: איך מעבירים את המפתח בבטחה? (סימטרי מול א-סימטרי)._
